...
async function getActiveLeads() {
  await connection()
  return sql`SELECT id, name, phone, car_id FROM leads WHERE status NOT IN ('ganho', 'perdido') AND deleted_at IS NULL ORDER BY name`
}
                        ...
                        <tr key={sale.id} className="border-b border-white/4 hover:bg-white/2">
                          <td className="px-4 py-3">
                            <p className="text-sm text-white">
                              {sale.car ? `${sale.car.brand} ${sale.car.model} ${sale.car.year}` : '—'}
                            </p>
                            {sale.lead && (
                              <p className="text-xs text-white/40">{sale.lead.name}</p>
                            )}
                          </td>
